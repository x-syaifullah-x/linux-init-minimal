...
# EROFS
mkfs.erofs -L ROOT --force-uid=0 --force-gid=0 "${x}${ROOTFS_IMAGE}" $ROOTFS_DIR || exit $?

cp -rfv $OUT_DIR/init $x
cp -rfv /media/xxx/sdc2/xxx/Git/x-syaifullah-x/linux-kernel-minimal/Microcode/Lenovo-B490/kernel $x || exit $?
