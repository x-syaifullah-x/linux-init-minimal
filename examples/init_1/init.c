...
void _start() {
    sys_c_mount(NULL, "/", NULL, MS_REC | MS_PRIVATE | MS_REMOUNT | MS_NOATIME | MS_SILENT, "size=100%");
    ...
    if (sys_c_fork() == 0) {
        ...
        if (md_assemble(sys_c_open(FS, O_RDONLY, NULL), MD_VERSION_1_2, disks, ARRAY_SIZE(disks)) >= 0) {
            if (sys_c_mkdir(O_LOWER, 0755) >= 0) {
                if (sys_c_mount(FS, O_LOWER, FS_TYPE, MS_RDONLY | MS_NOATIME | MS_SILENT, NULL) == 0) {
