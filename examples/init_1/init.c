...
void _start() {
    ...
    sys_c_mount(FS, SYSROOT, FS_TYPE, MS_RDONLY | MS_NOATIME, NULL);
    ...
    if (sys_c_mount("none", RUNTIME_DIR, "tmpfs", MS_NOSUID | MS_NODEV | MS_NOEXEC | MS_NOATIME | MS_SILENT, "size=100%,mode=0755") >= 0) {
        ...
        if (sys_c_fork() == 0) {
            ...
            sys_c_mount("none", "/usr", "overlay", MS_NOATIME | MS_SILENT, "lowerdir=/usr,upperdir=" O_UPPER "/usr,workdir=" O_WORK "/usr");
